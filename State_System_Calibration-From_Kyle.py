...
@author: kyleg
Updated on 2017-01-23, by dirktall04
'''
...
## Use the roadcenterlines from the FGDB instead.

#there are 25443 segments at this time

### Have to bring these into the file geodatabase if we want to include this in the processing,
### otherwise, this just needs to be done to the conflation dataset prior to using it for
### the rest of the processes. -- Transcend will be unable to run it using this connection.

## Maybe make some code that checks to see if the connections are available, and if so, redownloads
## a copy from this server before proceeding, but that otherwise continues normally with the 
## last version of the data that's in the FGDB.
...
fgdbSmlrs = '' # Get from the config file.
fgdbCmlrs = '' # Get from the config file.

from arcpy import (CopyFeatures_management, DeleteField_management, Describe,
    env, Exists,
    FeatureClassToFeatureClass_conversion, FeatureVerticesToPoints_management,
    LocateFeaturesAlongRoutes_lr, TableToTable_conversion)
    
env.overwriteOutput=1

# Update the fgdb versions with the sde versions, if available.
try:
    # Check to see if the Smlrs layer exists.
    # If so, then delete the fgdbSmlrs layer and replace with Smlrs from the network.
    # Then, do the same with the Cmlrs layer
    # and replace the fgdbCmlrs layer with it, if possible.
    pass
except:
    pass

# Let's Start by loading NG911 aggregated, conflated road centerlines to an in-memory feature class
# Then use the fgdb versions for the calculations.
...
Lrm_Dict = {'STATE':fgdbSmlrs, 'COUNTY':fgdbCmlrs}
...
## Rewrite this next part so that it writes each set of points to the FGDB and gives them
## easy to understand names that won't conflict with the calibration points created further
## down the line. -- Shouldn't be a problem if we keep these points in the *_Source.gdb.

# Find out which fields remain if you get a list of all the fields and then take out
# the following:
...
for key, value in Lrm_Dict.items():
    ...
    for end in End_List:
        ...
        # CopyFeatures_management(outtable, outstore)
...
## These points should not go back into the SQL server. Push them to the FGDB instead.
## They should then be used to calibrate the lines that are going to be used to create the
## LRS Routes, so update the State System's State Begin/End and County Begin/End measures
## from these points.